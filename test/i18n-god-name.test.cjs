...
test('every translation carries exactly the same keys as en', () => {
  const en = Object.keys(flatten(locale('en'))).sort();
  for (const l of LOCALES.filter((c) => c !== 'en')) {
    assert.deepEqual(Object.keys(flatten(locale(l))).sort(), en, `${l} drifted from en`);
  }
});

test('every {{placeholder}} in en survives in every translation', () => {
  // A translation that drops an interpolation renders a literal gap.
  const en = flatten(locale('en'));
  const vars = (v) => [...new Set((text(v).match(/\{\{(\w+)\}\}/g) || []))].sort();
  for (const l of LOCALES.filter((c) => c !== 'en')) {
    const tr = flatten(locale(l));
    const drift = Object.keys(en)
      .filter((k) => JSON.stringify(vars(en[k])) !== JSON.stringify(vars(tr[k])))
      .map((k) => `${k}: en=${vars(en[k])} ${l}=${vars(tr[k])}`);

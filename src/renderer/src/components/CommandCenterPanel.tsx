        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 1, minWidth: 0 }}>
            ...
            <span style={{
              ...
              whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'
            }}>{t('commandCenter.runsTheFloor')}</span>
          </div>
        </div>
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        {tab === 'terminal' && (
          isFullscreenedHere ? (
            ...
            <Centered>{t('commandCenter.noTerminal')}</Centered>
          )
        )}
...
function FloorTab({ seed }: { seed: { text: string; seq: number } }) {
  ...
  const dispatch = async () => {
    ...
    setDispatchMsg(res.ok
      ? suggested
        ? t('commandCenter.sentToWithSuggestion', { name: suggested.name })
        : t('commandCenter.sentToMichael')
  ...
  return (
    <Scroll>
      <Section title={t('commandCenter.dispatchViaMichael')}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
          ...
          <Select value={dispatchTo} onChange={setDispatchTo}>
            <option value="">{t('commandCenter.michaelDecides')}</option>
        ...
        <textarea
          ...
          rows={2}
          placeholder={t('commandCenter.dispatchPlaceholder')}
          style={textareaStyle}
        />
                ...
                <PixelButton
                  ...
                  onClick={async () => {
                    ...
                    if (engineProvider !== currentProvider) {
                      if (!window.confirm(t('commandCenter.confirmRestartEngine'))) return;
                    }

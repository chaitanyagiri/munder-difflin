...
function Header({ agent }: { agent: Agent }) {
  ...
  const onKill = async () => {
    ...
    if (!confirm(`Close ${agent.name}? The PTY process will terminate and the agent is archived (kept in history, off the precinct floor).`)) return;

...
**Write the ask in markdown.** The card renders it, so plain-text asterisks and backticks show up
literally and a wall of text is unreadable:
- open with a short **bold** line saying what you need;
- \`backticks\` for paths, commands, values, and identifiers;
- \`-\` bullets or \`1.\` numbering whenever there is more than one option or step;
- a blank line between paragraphs; a single newline is rendered as a line break, so each option
  stays on its own line.

Keep it short and answerable. Do NOT park human questions in separate files (no \`HumanQuestion.md\`),

...
export class HiveManager {
  ...
  private hookSettings(shim: string, cwd: string, cfg: McpDefaultsMap, theme?: 'light' | 'dark', writableDirs: string[] = [], allow?: string[]): unknown {
  ...
  private buildDefaultMcpServers(
    ...
    // When an allowlist is provided (worker-scoped spawn), only inject the
    // explicitly requested servers. This eliminates 100–200+ tool-schema tokens
    // per unneeded server (Figma: 42, ClickUp: 58, Notion: 43, Canva: 34 …).
    ...
    for (const e of MCP_CATALOG) {
      // Allowlist gate: if the caller specified mcpAllow, skip any server not in it.
      if (allowSet && !allowSet.has(e.id)) continue;
   ...
   */
  private copyBundledSkills(srcDir: string, destDir: string, allow?: string[]): void {
    try {
      if (!existsSync(srcDir)) return;
      // When an allowlist is provided (worker-scoped spawn), copy only those skill
      // directories. This shrinks the per-session skill-catalog bootstrap payload
      // for workers that do not need the full 100+ skill set.
      const allowSet = allow && allow.length > 0 ? new Set(allow) : null;
      ...
      if (allowSet) {
        // Selective copy: only top-level skill directories in the allowlist.
        const entries = readdirSync(srcDir, { withFileTypes: true });
        mkdirSync(destDir, { recursive: true });
        for (const ent of entries) {
          if (ent.isDirectory() && allowSet.has(ent.name)) {
            copyTree(join(srcDir, ent.name), join(destDir, ent.name));
          }
        }
      } else {

...
export class HiveManager {
  ...
  private injectedPrompt(
    ...
    const godLine = meta.isGod
      ...
      : meta.isAssistant
      ? 'You are Captain Holt\'s PREP ASSISTANT. You will be handed short, possibly vague instructions (each begins with "ENRICH TASK:"). For each one: (1) figure out which project it concerns and cd into the most relevant repo — you start in Captain Holt\'s home directory; (2) gather concrete context READ-ONLY (exact file paths, current state, relevant code, conventions, active branch, gotchas) — NEVER modify, create, or delete files; (3) rewrite the instruction into ONE clear, self-contained prompt that Captain Holt can execute autonomously, preserving the user\'s original intent without inventing scope. Then deliver it: write ONE message JSON into your outbox with "to":"god", "act":"request", a short subject, and the finished prompt as the body. Do NOT perform the task yourself — your only output is the improved prompt sent to Captain Holt.'
      : 'For anything ambiguous, cross-cutting, or needing sign-off, address a message to "god".';
    const commandLine = godLine
      .replace('GOD / ORCHESTRATOR of this hive', 'COMMAND CAPTAIN / ORCHESTRATOR of this precinct')
      .replace('hive agents', 'precinct agents')
      .replace("your hive's sibling agents", "your precinct's sibling agents")
      .replace("office floor's ASK ME board", "precinct floor's ASK ME board");
    const guardrailsLine = 'Guardrails: a circuit breaker watches the precinct floor — a "Circuit breaker: steer/constrain" message means you are looping or overspending, so STOP repeating, summarize what you tried, and follow it. Be token-frugal (a floor-wide or per-agent token budget can pause you). The shared plan has two parts: board.md (freeform; command is the sole scribe) and tasks.json (structured kanban — todo/doing/blocked/done).';
    ...
    return [
      `You are "${meta.name}" (${meta.id}), an autonomous agent in The Precinct's collaborating team of Claude agents.`,

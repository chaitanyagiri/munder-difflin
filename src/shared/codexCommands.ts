...
export const CODEX_COMMAND_GROUPS: CmdGroup[] = [
  ...
  },
  {
    ...
    items: [
      { cmd: 'codex --dangerously-bypass-approvals-and-sandbox', kind: 'cli', desc: 'Auto mode: skip ALL approval prompts AND drop the OS sandbox (full filesystem access). Used by The Precinct when auto mode is on — full parity with Claude bypassPermissions, and required so an agent can write to its folder outside the project cwd.' },
  ...
  },
  {
    ...
    items: [
      ...
      { cmd: 'CODEX_NON_INTERACTIVE=1 codex', kind: 'cli', desc: 'Suppress all interactive installer / first-run prompts. Set automatically by The Precinct in auto mode.' }
    ]
  },
  {
    title: 'CONFIG',
